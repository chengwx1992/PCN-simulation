...
namespace ns3 {
		...
		TimelySender::Init()
	{
		...
		m_N = 1;
		negGradientCount = 0;
		lastUpdateTime = Simulator::Now().GetDouble();
	}
		...
		TimelySender::Send(void)
	{
		...
		if (m_sent % m_burst_in_packets == 0)
		{
			m_sleep = GetBurstDuration(m_rate);
		}
		else {
		...
		next_avail = next_avail>m_sleep ? next_avail : m_sleep;
		...
		if ((m_sent + 1) % (int)m_burst_in_packets == 0)
			seqTs.SetAckNeeded();
		Ptr<Packet> p = Create<Packet>(m_pktSize - 14 - 10); // 14 : the size of the seqTs header, 10: the size of qbb header
		p->AddHeader(seqTs);
		...
		if (m_sent < m_allowed)
		{
			...
			m_sendEvent = Simulator::Schedule(Seconds(next_avail), &TimelySender::Send, this);
		}
		...
		TimelySender::Receive(Ptr<Socket> socket)
	{
		...
		{
			m_received++;
			//printf("Time=%d, Node %d Recv %d ", Simulator::Now().GetMicroSeconds(), m_node->GetId(), m_received);
			int x = packet->GetSize();
			SeqTsHeader seqTs;
			packet->RemoveHeader(seqTs);
			m_new_rtt = GenerateRTTSample(seqTs.GetTs());
			//printf("seq=%d TS=%d rtt=%f\n", seqTs.GetSeq(), seqTs.GetTs().GetMicroSeconds(), m_new_rtt);
			UpdateSendRate();
		}
	}
		...
		TimelySender::UpdateSendRate()
	{
		...
		double delta_factor = (Simulator::Now().GetSeconds() - lastUpdateTime) / m_min_rtt;
		delta_factor = std::min(delta_factor, 1.0);

		double m_newRate;
		char *updaterule;
		if (m_new_rtt < m_t_low)//additivive increase if rtt < t_low
		{
			updaterule = "t_low";
			m_newRate = m_rate + m_delta*delta_factor;
		}
		else if (m_new_rtt > m_t_high)//multiplicative decrease if rtt > t_high
		{
			updaterule = "t_high";
			m_newRate = m_rate * (1 - delta_factor*m_beta * (1 - m_t_high / m_new_rtt));
		}
		else if (normalized_gradiant < 0)
		{
			updaterule = "negative_grad";
			m_N = 1;
			if (negGradientCount >= 5)
				m_N = 5;
			m_newRate = m_rate + m_N * m_delta*delta_factor;
		}
		else
		{
			updaterule = "pos_grad";
		...
		m_newRate = std::max(m_newRate, m_rate * 0.5);
		m_newRate = std::min(m_newRate, m_maxRate);
		m_newRate = std::max(m_newRate, m_minRate);

		//printf("%f Z%d %s %f %f\n", Simulator::Now().GetSeconds(), m_node->GetId(), \
					updaterule, m_new_rtt * 1000 * 1000, m_newRate / 1000.0 / 1000.0);

		m_rate = m_newRate;
		m_prev_rtt = m_new_rtt;
		//m_sleep = GetBurstDuration(m_rate);
		lastUpdateTime = Simulator::Now().GetSeconds();
